...
__all__ = ['get_boxscore', 'get_all_boxscores_from_linescore_page', 'get_full_boxscore', 'get_game_info', 'hash_game',
           'get_page_soup', 'request_type']
...
def get_all_boxscores_from_linescore_page(
    ...
    boxscores = [_get_boxscore(soup=soup,soup_type=soup_type,game_number=i+1) for i in range(game_numbers)]

    draw = _get_draw(soup=soup,soup_type = soup_type,**draw_kwargs)

    # will utf-8 always work?
    hashes = [sha256(str(boxscore).encode('utf-8')).hexdigest() for boxscore in boxscores]

    scores = list()
    for boxscore, _hash in zip(boxscores,hashes):
        new_boxscore = dict()
...
def get_full_boxscore(
    ...
    # will utf-8 always work?
    _hash = hash_game(game_dict = boxscore)
...
def hash_game(
    ...
    return hash_func(str(game_dict).encode(encoding)).hexdigest().lower()
